...
;" >
        <q-card-section>
          ...
          <div class="q-gutter-md">
            <q-input v-model="store.currentItem.name" label="Nama Barang" outlined clearable />
            <q-input v-model.number="store.currentItem.JumlahBarang" type="number" label="Jumlah Barang" outlined clearable />
            <q-input v-model.number="store.currentItem.BeratBarang" type="number" label="Berat Per Barang (g)" outlined clearable />
            <q-input v-model.number="store.currentItem.HargaBarang" type="number" label="Harga Per Barang" outlined clearable />
            <q-input v-model.number="store.currentItem.protein" type="number" label="Protein (g)" outlined clearable />
            <q-input v-model.number="store.currentItem.sodium" type="number" label="Natrium (mg)" outlined clearable />
          ...
          <q-markup-table separator="horizontal" flat bordered class="rounded-borders">
            <thead>
              <tr class="bg-secondary text-white">
                ...
                <th class="text-right text-h6">Protein (g)</th>
                <th class="text-right text-h6">Natrium (mg)</th>
                <th class="text-center text-h6">Aksi</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in store.items" :key="item.id" class="text-grey-9">
                ...
                <td class="text-center">
                  <q-btn size="md" flat icon="edit" color="blue-6" @click="store.editItem(item)" round />
                  <q-btn size="md" flat icon="delete" color="red-6" @click="store.deleteItem(item.id)" round class="q-ml-sm" />
                </td>
              </tr>
